...
def get_team_info(server_version: int) -> str:
    ...
    global newMatch
    newMatch = True
    return 'YL5'             # 在此行修改双引号中的字符串为自己的队伍名
...
def goalkeeper (field):
    ...
    # 在己方
    if futureBallx <= 0 :
        if distance_ball <= 39 or (20 <= abs(futureBally) <= 60 and futureBallx <= -87) :  # 安全的情况下守门员出去踢球
            if num_in_goal <= 2 :
                ...
                baseRobots[0].goalkeeper_throw_ball(football_now_x , football_now_y)
        elif abs(futureBally) < 25 :
            baseRobots[0].moveto(GlobalVariable.goalkeeper_X , futureBally)
            baseRobots[0].goalkeeper_throw_ball(football_now_x , football_now_y)
        elif abs(futureBally) > 60 and futureBallx <= -87 :
            if futureBally > 0 :
                if num_in_goal <= 1 :
                    baseRobots[0].moveto(futureBallx , futureBally)
                    baseRobots[0].goalkeeper_throw_ball(football_now_x , football_now_y)
