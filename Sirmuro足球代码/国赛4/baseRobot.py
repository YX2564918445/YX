...
class BaseRobot:
    def __init__(self):
        ...
        self.pid_moveto = PID(1.96, 0, 1.62)
        self.pid_keepermoveto = PID(1.91,0,1.54)
        self.pid_moveto_dis = PID(2.48, 0, 6.06)
        self.pid_angle = PID(1.6, 0, 4.2)
        self.pid_goalkeeper_moveto = PID(1.86,0,1.52)
    ...
    def keepermoveto(self, tar_x, tar_y):  # 横向移动pid跑位函数
        if self.tick == 1 or self.tick == 2 or self.tick == 3 or self.tick % 100 == 0 or self.reset is True:
            self.pid_moveto.reset_pid()
        v_max = 125
        dx = tar_x - self.PredictInformation[GlobalVariable.tick_delay].position.x
    ...
    def goalkeeper_moveto(self, tar_x, tar_y):  # 守门员pid跑位函数
        if self.tick == 1 or self.tick == 2 or self.tick == 3 or self.tick % 100 == 0 or self.reset is True:
            self.pid_moveto.reset_pid()
        v_max = 125
        dx = tar_x - self.PredictInformation[GlobalVariable.tick_delay].position.x
