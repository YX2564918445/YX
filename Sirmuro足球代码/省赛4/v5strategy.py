...
def get_team_info(server_version: int) -> str:
    ...
    global newMatch
    newMatch = True
    return 'YL3'             # 在此行修改双引号中的字符串为自己的队伍名
...
def goalkeeper (field):
    ...
    # 在己方
    if futureBallx <= 0 :
        if (futureBallx <= -75 and abs(futureBally) <= 40) or futureBallx <= -90 :  # 安全的情况下守门员出去踢球
            if num_in_goal <= 2 :
                baseRobots[0].moveto(futureBallx , futureBally)
                baseRobots[0].goalkeeper_throw_ball(football_now_x , football_now_y)
            else :
                baseRobots[0].moveto(GlobalVariable.goalkeeper_X , futureBally)
                baseRobots[0].goalkeeper_throw_ball(football_now_x , football_now_y)
        elif abs(futureBally) <= 35 :
            baseRobots[0].moveto(GlobalVariable.goalkeeper_X , futureBally)
        elif futureBallx <= -75 :
            if futureBally > 0 :
                baseRobots[0].moveto(GlobalVariable.goalkeeper_X , 19)
            else :
                baseRobots[0].moveto(GlobalVariable.goalkeeper_X , -19)
        elif 35 < abs(futureBally) <= 50 :
            if futureBally > 0 :
                baseRobots[0].moveto(GlobalVariable.goalkeeper_X , 6)
            else :
                baseRobots[0].moveto(GlobalVariable.goalkeeper_X , -6)
        else :
            if futureBally > 0 :
                baseRobots[0].moveto(GlobalVariable.goalkeeper_X , 8)
            else :
                baseRobots[0].moveto(GlobalVariable.goalkeeper_X , -8)
    # 在对方
    else :
        if abs(futureBally) <= 35 :
            baseRobots[0].moveto(GlobalVariable.goalkeeper_X , 0)
        elif futureBally > 0 :
            baseRobots[0].moveto(GlobalVariable.goalkeeper_X , 6)
        elif futureBally < 0 :
            baseRobots[0].moveto(GlobalVariable.goalkeeper_X , -6)
...
def tick_goalkeeper (field):
    ...
    if tick == 1:  #守门线
        if GlobalVariable.tick - tickBeginPenalty <= 50:
            flag_tick = 1  # 换策略
            baseRobots[0].moveto(GlobalVariable.goalkeeper_X , futureBally * 1.1) #直上直下
...
def strategy_common(field):
    ...
    '4号一直在己方后半场做直线防守'
    # 1号机器人（全场自由人）
    if futureBallx <= -70 :                                                  # 突破了所有防线交给守门员
        if futureBally >= 45 :
            baseRobots[1].moveto( -68 , 50)
        elif futureBally <= -45 :
            baseRobots[1].moveto( -68 , -50)
        else :
    ...
    elif futureBallx <= 85 :                                                # 在对方小禁区x外追球
        baseRobots[1].moveto(futureBallx , futureBally)
        baseRobots[1].throw_ball(football_now_x , football_now_y)
    else :                                                                  # 在对方小禁区x内
        if abs(futureBally) >= 30 :  # 在对方小禁区y外，追球
            ...
            baseRobots[1].throw_ball(football_now_x , football_now_y)                                                     # 横向移动

    # 2号机器人左前锋
    if futureBallx < 10 :                                                   # 球在己方，定点不动
        baseRobots[2].moveto( 5 , 40)
    elif futureBally >= -20 :                                               # 球在上半区配合自由人追球
        if futureBallx <= 83 :                                              # 球在对方小禁区x范围外追球
            ...
            baseRobots[2].moveto( GlobalVariable.keeper_x2 , futureBally)
            baseRobots[2].throw_ball( football_now_x , football_now_y)
...
def strategy_goalkick(field):
    ...
    if race_state_trigger == Team.Self:
        if goal == 1:                                   #二传
            ...
            elif GlobalVariable.tick - tickBeginGoalKick <= 48 :
                for i in range(1 , 4) :
                    baseRobots[i].set_wheel_velocity(0 , 0)
                baseRobots[1].set_wheel_velocity(-125 , 125)
    ...
    if race_state_trigger == Team.Opponent:
        if guard_goal == 1:                 # 1号横向移动，其他追球
            ...
            if GlobalVariable.tick - tickBeginGoalKick <= 90 :
                goalkeeper(field)
                if GlobalVariable.tick - tickBeginGoalKick <= 50 :
                    if futureBally <= -70 :
                        baseRobots[1].moveto(-70 , futureBally + 5)
                        baseRobots[2].moveto(-70 , futureBally - 5)
                        baseRobots[3].moveto(-70 , futureBally * 1.1)
                        baseRobots[4].moveto(-70 , -futureBally * 1.1)
                    elif futureBallx < 0 :
                        baseRobots[1].shoot(futureBallx , futureBally)
                        baseRobots[2].shoot(futureBallx , futureBally)
                        baseRobots[3].shoot(futureBallx , futureBally * 1.1)
                        baseRobots[4].shoot(futureBallx , futureBally * 1.1)
                    elif futureBallx >= 0 :
                        baseRobots[1].set_wheel_velocity(0 , 0)
                        baseRobots[2].set_wheel_velocity(0 , 0)
                        baseRobots[3].set_wheel_velocity(0 , 0)
...
def strategy_PlaceKick (field): #开球策略             开球改动地方
    ...
    # 1号球员到球的距离
    dx1 = futureBallx - baseRobots[4].get_pos().x
    dy1 = futureBally - baseRobots[4].get_pos().y
    distance4 = math.sqrt(dx1 * dx1 + dy1 * dy1)
    # 攻方策略
    if race_state_trigger == Team.Self:
        if faceoff == 1 :  # 向己方踢反弹
            for i in range(4 , 5) :
            ...
            if GlobalVariable.tick - tickBeginPlaceKick <= 70 :
                ...
                goalkeeper(field)
                baseRobots[2].moveto(75 , -40)
                baseRobots[3].moveto(75 , -30)
            else :
            ...
            for i in range(1 , 4) :
                baseRobots[i].set_wheel_velocity(0 , 0)
            goalkeeper(field)
            if GlobalVariable.tick - tickBeginPlaceKick <= 33 :
                baseRobots[1].set_wheel_velocity(125 , 125)
                baseRobots[3].set_wheel_velocity(100 , 100)
            elif GlobalVariable.tick - tickBeginPlaceKick <= 34 :
                baseRobots[1].set_wheel_velocity(-125 , 125)
                baseRobots[3].set_wheel_velocity(100 , 125)
            elif GlobalVariable.tick - tickBeginPlaceKick <= 66 :
                baseRobots[1].set_wheel_velocity(-125 , 125)
                baseRobots[3].set_wheel_velocity(100 , 125)
                baseRobots[2].set_wheel_velocity(125 , 125)
            elif GlobalVariable.tick - tickBeginPlaceKick <= 85 :
                baseRobots[1].set_wheel_velocity(-125 , 125)
                baseRobots[3].set_wheel_velocity(100 , 125)
                baseRobots[2].set_wheel_velocity(125 , -125)
            else:
    ...
    if race_state_trigger == Team.Opponent:
        if faceoff_defense == 1 :                       # 一动三静型防守
            ...
            if GlobalVariable.tick - tickBeginPlaceKick <= 90 :
                flag_faceoff_defense = 1  # 准备换策略
                goalkeeper(field)
                if GlobalVariable.tick - tickBeginPlaceKick <= 50 :
                    if futureBallx <= -70 :
                        baseRobots[1].moveto(-70 , futureBally + 5)
                        baseRobots[2].moveto(-70 , futureBally - 5)
                        baseRobots[3].moveto(-70 , futureBally * 1.1)
