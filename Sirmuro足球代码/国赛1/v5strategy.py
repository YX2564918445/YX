...
def get_team_info(server_version: int) -> str:
    ...
    global newMatch
    newMatch = True
    return 'YL_1'             # 在此行修改双引号中的字符串为自己的队伍名
...
def predicted_position(field) :
    ...
    futureBallx = 5 * football_now_x - 4 * BallPos[GlobalVariable.tick - 1].x
    futureBally = 5 * football_now_y - 4 * BallPos[GlobalVariable.tick - 1].y
    goalkeeper_futureBallx = 9 * football_now_x - 8 * BallPos[GlobalVariable.tick - 1].x
    goalkeeper_futureBally = 9 * football_now_y - 8 * BallPos[GlobalVariable.tick - 1].y
