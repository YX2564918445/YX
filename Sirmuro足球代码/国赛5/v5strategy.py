...
def get_team_info(server_version: int) -> str:
    ...
    global newMatch
    newMatch = True
    return 'YL_5'             # 在此行修改双引号中的字符串为自己的队伍名
...
def strategy_PlaceKick (field): #开球策略             开球改动地方
    ...
    if race_state_trigger == Team.Opponent:
        if faceoff_defense == 1 :                       # 西建
            ...
            if GlobalVariable.tick - tickBeginPlaceKick <= 110 :  # 仿空大开球防守
                goalkeeper(field)
                ...
                if GlobalVariable.tick - tickBeginPlaceKick <= 20 :
                    ...
                    if football_now_x <= -75 :
                        strategy_common(field)
                    elif futureBallx <= 20 :
                        for i in range(1 , 5) :
                            baseRobots[i].moveto(futureBallx , futureBally)
                    else :
                        for i in range(1 , 5) :
                            baseRobots[i].set_wheel_velocity(0 , 0)
            else :
