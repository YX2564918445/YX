...
def get_team_info(server_version: int) -> str:
    ...
    global newMatch
    newMatch = True
    return 'YL2'             # 在此行修改双引号中的字符串为自己的队伍名
...
def goalkeeper (field):
    ...
    # 在己方
    if futureBallx <= 0 :
        if (futureBallx <= -75 and abs(futureBally) <= 40) or futureBallx <= -91 :  # 安全的情况下守门员出去踢球
            if num_in_goal <= 2 :
    ...
    if race_state_trigger == Team.Opponent:
        if guard_goal == 1:                 # 1号横向移动，其他追球
            ...
            if GlobalVariable.tick - tickBeginGoalKick <= 88 :
                ...
                baseRobots[1].moveto(-70 , 30)
                baseRobots[4].moveto(-70 , -30)
            else :
                strategy_common(field)  # 调用常规策略
